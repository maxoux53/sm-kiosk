    ...
    try {
        ...
        if (sanitizedSearch) {
            if (!isNaN(Number(sanitizedSearch))) {
                ...
                orConditions.push({ id: { equals: parsedNumericSearch } });
            } else {
                orConditions.push(
                    {
                        first_name: {
        ...
        for (const iUser in users) {
            if (users[iUser].avatar) {
                users[iUser].avatar =
                    `https://imagedelivery.net/${process.env.CF_ACCOUNT_HASH}/${users[iUser].avatar}/public`;
            }
        }
...
export const updateUser = async (
    ...
    const password_hash =
        req.body.password ? await hash(req.body.password) : undefined;
